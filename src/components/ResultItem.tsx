  ...
  return (
    <li className='flex justify-center border-solid border-black border-2 flex-col mx-24 my-4 rounded-lg max-w-xs h-[500px] shadow-boxMain hover:shadow-cardBox bg-white'>
      ...
      <div className='flex flex-wrap w-full h-[60px] items-center justify-center mb-2'>
        {filterType && (
          <div className='flex justify-center items-center p-[3px] max-w-1200px} max-h-{30px} my-0.5 mx-1 text-mainVeryLighter bg-black rounded-lg text-base'>
            {filterType.charAt(0).toUpperCase() +
              filterType.slice(1).toLowerCase()}
          </div>
        )}
        {filterBedrooms > 0 && (
          <div className='flex justify-center items-center p-0.5 max-w-1200px} max-h-{30px} my-0.5 mx-1 text-mainVeryLighter bg-black rounded-lg text-base'>
            {filterBedrooms === 1
      ...
      <div className='flex justify-center items-center w-full h-[60px] bg-mainDarker'>
        <h3 className='text-2xl text-green font-sans bg-black p-1 rounded-xl'>
          {price}
        </h3>
      </div>
      ...
      <div className='w-full h-fit mb-0'>
        <Link
          to={`/results/${id}`}
          className='flex font-sans text-lg justify-center items-center bg-black no-underline text-white rounded-sm py-2 hover:text-mainDarker transition-color duration-500'
        >
          Mais Detalhes
        </Link>
      </div>
